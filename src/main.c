        ...
        for (int j = 0; j < conf->watchers[i].types_length; j++) {
            ...
            if (strcmp(ext, conf->watchers[i].file_types[j]) == 0) {
                ...
                if (rename(old_path, new_path) == -1) {
                    logger(
                        WARNING,
                        "Error occured while moving file: %s",
                        strerror(errno)
                    );
                } else {
                    logger(
...
int main(int argc, char **argv)
{
    ...
    char *pid_file = NULL;
    char *default_home = NULL;
    int val, option_index = 0;

    while ((val = getopt_long(
        argc, argv, "c:l:p:d:h", long_options, &option_index)) != -1) {
        switch (val) {
        case 'c':
    ...
    if (conf == NULL || init_notify(conf) == 1) goto end;
    ...
    //Main loop
    while (running) {
        if (get_event(conf, move_file) == 1) break;
    }
    ...
    close_log();
    FREE(log_file, config_path, pid_file, default_home);
